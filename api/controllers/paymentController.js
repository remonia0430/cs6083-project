...
const makePayment = async (req, res) => {
    ...
    if (amount === null) return res.status(400).json({ success: false, message: 'missing payment amount' });
    if (!rentId) return res.status(400).json({ success: false, message: 'missing rent id' });
    if (method === 'Credit' && (!fname || !lname)) return res.status(400).json({ success: false, message: 'missing card holder name' });
